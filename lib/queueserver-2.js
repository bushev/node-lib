...
class QueueServer2 {
    ...
    constructor() {
        ...
        this._client = kue.createQueue({
            ...
            kue.Job.get(id, (err, job) => {
                ...
                this.logger.debug(`QueueServer2:[job complete]: ${job}, ${JSON.stringify(result)}`);
    ...
    checkWorkerCommand(workerName, commandName) {
        ...
        const worker = require('.' + this._workersDir + '/' + workerName + '.js');
    ...
    initWorkers() {
        ...
        this._workers.filter(file => {
            ...
            try {
                ...
                throw new Error(`QueueServer2::initWorkers: Unable to load file "${file}".`);
            }
